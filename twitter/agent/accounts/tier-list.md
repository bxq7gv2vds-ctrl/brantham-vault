---
type: agent-config
updated: 2026-03-26
last_scan: 2026-03-26
---

# Tier List — Comptes a suivre

## Tier 1 — Etudier en detail (style, hooks, timing)
- @levelsio — indie hacking + AI, direct, build in public avec chiffres
- @swyx — AI engineer, mix technique/accessible
- @steipete — power user Claude Code, builder narrative
- @karpathy — technique mais accessible, engagement massif
- @alexalbert__ — Claude Code creator
- @bindureddy — enterprise AI, opinions fortes
- @buccocapital — observations AI/work, 1.1K likes sur Claude Cowork take (26/03). Ideal pour reply.
- @RoundtableSpace — mega-compte news AI, 80K views/tweet. Breaking news = reply strategique.

## Tier 2 — Veille quotidienne
- @_akhaliq — papers daily, toujours premier
- @therundownai — newsletter AI, curation
- @gregisenberg — startup ideas
- @WatcherGuru — breaking news
- @DeItaone — breaking news rapide
- @noahzweben — Anthropic engineer, annonces Claude Code officielles
- @lydiahallie — dev, react aux news Claude
- @poetengineer__ — Claude Code power user, dashboards, 254 sessions

## Tier 2-FR — Comptes francais tech/AI (decouverts 26/03)
- @Frenchiee — react FR aux news Claude/AI, franglais
- @melvynx — builder FR, commente tools en temps reel
- @0xmaxou — humor tech FR, Claude user
- @BrivaelFr — hype startups YC, agents AI
- @thismacapital — finance + sarcasme FR

## Tier 3 — A decouvrir / observer
- @arafatkatze — multi-agent orchestration, 427 likes sur Cline take
- @zostaff — builder bots, 209 bookmarks sur Polymarket bot
- @sawyerhood — dev-browser CLI, Playwright agent approach
- @plbiojout — NanoCorp (YC), AI autonomous company
- @enfantdlajungle (Mowgli) — FR, engagement decent

## Regles
- Revisiter chaque semaine
- Max 2-3 interactions/jour par compte
- Promouvoir Tier 3 → Tier 2 si 3+ apparitions dans le feed
- Nouveau : section Tier 2-FR pour l'audience francaise

## Related
- [[_system/MOC-master]]
